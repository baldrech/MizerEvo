...
cohortSpan <- c(2900,3100,4000,4500)
myData <- plotCohortLong(folder = folder, cohortSpan = cohortSpan, t_steps = 10, returnData = T )
saveRDS(myData,file = "fitnessMat7.rds")
myData <- readRDS("fitnessMat7.rds")
...
png(filename=paste(folder,"/Fitness.png",sep=""), width = (8*(length(plotSpList[[1]])+1)), height = (8*(length(plotSpList)+1)), units = "cm",res = 600)
